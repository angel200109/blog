...
有些网站为了防止调试会重写 `console.log`——要么清空它，要么改成什么都不做的空函数。但你可能还是想用原生的 console 来排查问题。怎么拿回来？
...
生产环境遇到 console.log 被屏蔽，通常是因为构建工具配了 `drop_console` 或者监控脚本动了手脚。这种情况不建议在生产环境恢复——日志可能会泄露敏感信息。

开发/测试环境的话：

```js
// 在页面上方挂个保留引用
window.__originalConsole = console.log.bind(console)

// 以后在控制台直接调用
window.__originalConsole('我还在')
```

有一些前端监控 SDK 也会在你不知情的情况下劫持 console。比如 Sentry 会收集 console 日志用于错误追踪，但它保留了原始方法的引用，不会影响正常输出。

## 总结

iframe 大法是最通用的恢复方式，前提是你能在当前页面跑 JS。原型链恢复看情况，不一定好使。调试神器是 DevTools 的 context 切换。写自己的代码时，可以提前存个引用以防万一。
